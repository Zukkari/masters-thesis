...
\subsection{Definitions of investigated code smells}\label{subsec:definitions-of-investigated-code-smells}

\TODO{
Here we need to describe definitions of code smells that we investigated.
Problem: code smells were defined by Kristiina, but no paper has been published yet?
}
...
\subsection{Analysis tools}\label{subsec:analysis-tools}

\TODO{
Describe tools to be used in analysis.
}

\subsubsection{SonarQube plugin}

\TODO{
Describe plugin for SonarQube (what does it do, how does it work, what can be configured, show end user can
user the plugin, architecture of the plugin).
What do consider: how can the user select what rules to enable, context of a single rule (severity etc)
}

\subsubsection{SonarQube project bulk analyzer}

\TODO{
Describe the tool that was developed for bulk project analysis.
Describe its functionality:
\begin{itemize}
    \item Parse the input corpus file
    \item Clone the git projects
    \item Build the projects (using gradle/maven), here we can also mention that we use wrapper if it is provided with the
    project and if the wrapper is not provided we use default version installed on the system.
    \item In case of Gradle, we have to modify the build script to include the SonarQube analysis plugin.
    \item Create projects in the SonarQube server (with configured profile)
    \item Run the analysis on the projects (data is uploaded by the SonarQube plugin itself)
    \item To implement: fetching results from the SonarQube once all of the projects are analyzed.
\end{itemize}
}
